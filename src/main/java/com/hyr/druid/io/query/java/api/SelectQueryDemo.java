 ...
 * @Description: select查询 http://druid.io/docs/0.10.1/querying/select-query.html
 ******************************************************************************/
public class SelectQueryDemo {
    ...
    public static void main() throws IOException {
        ...
        filters.add(DimFilters.dimEquals("colo", "phx")); // 拦截 colo='phx'

        Map<String, Integer> map = new HashMap<String, Integer>();
        PagingSpec pagingSpec = new PagingSpec(map, 3); // 返回Top 3个结果

        Map<String, Object> contextConfig = new HashMap<String, Object>(); // 优化配置

        List<String> dims = Arrays.asList("colo", "pool", "report", "URL", "TS", "metric", "value", "count", "min", "max", "sum");

        SelectQuery query = new Druids.SelectQueryBuilder()
                .dataSource("test")
                .intervals(QuerySegmentSpecs.create(new Interval(0, new DateTime().getMillis()))) // 时间间隔
                .granularity(Granularity.fromString("DAY")) // 时间粒度
                .pagingSpec(pagingSpec) // 返回结果相关设置
                .dimensions(dims) // 查询字段
                .filters(DimFilters.and(filters)) // where筛选
