 ...
 * @Description: 根据时间维度查询
 ******************************************************************************/
public class TimeseriesQueryDemo {
    ...
    public static void main2() throws IOException {
        ...
        List<AggregatorFactory> aggregatorFactoryList = new ArrayList<AggregatorFactory>();
        aggregatorFactoryList.add(new LongSumAggregatorFactory("agg_count", "agg_count"));
        aggregatorFactoryList.add(new LongMaxAggregatorFactory("agg_max", "agg_max"));
        aggregatorFactoryList.add(new LongMinAggregatorFactory("agg_min", "agg_min"));
        aggregatorFactoryList.add(new DoubleSumAggregatorFactory("agg_sum", "agg_sum"));

        List<PostAggregator> postAggregatorList = new ArrayList<PostAggregator>();

        TimeseriesQuery query = new TimeseriesQuery(dataSource,
        ...
        TimeseriesQueryQueryToolChest toolChest = new TimeseriesQueryQueryToolChest(Utils.NoopIntervalChunkingQueryRunnerDecorator());
        TimeseriesQueryEngine engine = new TimeseriesQueryEngine();

        QueryWatcher NOOP_QUERYWATCHER = new QueryWatcher() {
            public void registerQuery(Query query, ListenableFuture future) {
            }
        };

        TimeseriesQueryRunnerFactory factory = new TimeseriesQueryRunnerFactory(toolChest, engine, NOOP_QUERYWATCHER);

        Sequence<Result<TimeseriesResultValue>> resultSequence = factory.createRunner(new QueryableIndexSegment("", index)).run(query, null);

        ArrayList<Result<TimeseriesResultValue>> results = Sequences.toList(resultSequence, Lists.<Result<TimeseriesResultValue>>newArrayList());

        System.out.println("=========================================");
        for (Result<TimeseriesResultValue> r : results) {
